...
function handleMouseDown(event){
  var position = boardPostion(event.clientX, event.clientY);
  var x = position.x;
  var y = position.y;
  ...
  if(state.board[y][x] && state.board[y][x].charAt(0) === state.turn) {
    ...
    state.movingPiece = {
      ...
      startPostion: {x: x, y: y},
      currentPosition: boardPostion(event.clientX, event.clientY)
    }
...
function handleMouseUp(event){
  ...
  var y = position.y;
  if(x < 0 || y < 0 || x > 9 || y > 9){
    //Release off board; rubberband back to startPostion
    var sx = state.movingPiece.startPostion.x;
    var sy = state.movingPiece.startPostion.y;
...
function renderDragging(){
  ...
  ctx.beginPath();
  ctx.arc(
    state.movingPiece.startPostion.x*100+50,
    state.movingPiece.startPostion.y*100+50,
    40, 0, Math.PI * 2
  );
  ctx.fill();
}
function handleMouseMove(event) {
  ...
  switch(state.action) {
    ...
    case 'dragging':
      //Render ghost
      state.movingPiece.currentPosition = boardPostion(event.clientY, event.clientX);
      renderDragging();
